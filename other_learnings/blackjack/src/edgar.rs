...
impl Player {
    ...
    pub fn edgar_turn(&mut self, deck: &mut Deck, dealer_showing: Value, header: Header) {
        ...
        while self.hand.value > 0 {
            ...
            match self.hand.get_type() {
                ...
                HandType::AceSix => {
                    if dealer_showing.numeric_val() >= 3
                        && dealer_showing.numeric_val() <= 6
                        && self.wager * 2.0 <= self.funds
                    {
                        self.double_down(deck, false);
                ...
                HandType::AceFive => {
                    if dealer_showing.numeric_val() >= 4
                        && dealer_showing.numeric_val() <= 6
                        && self.wager * 2.0 <= self.funds
                    {
                        self.double_down(deck, false);
