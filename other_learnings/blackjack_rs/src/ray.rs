...
impl Player {
    ...
    pub fn ray_turn(&mut self, deck: &mut Deck, dealer_showing: Value, count: i8) {
        sleep(3);
            ...
            match self.hand.get_type() {
                ...
                HandType::ValueBased => {
                    ...
                    if self.hand.value >= 17 {
                        if p_count <= -5 && dealer_showing.numeric_val() >= 7 {
                            self.hit(deck, false);
                        } else {
                            print_if(format!("{} stood!", self.name).as_str());
                            return;
                        ...
                        if dealer_showing.numeric_val() >= 10
                            || (dealer_showing.numeric_val() >= 7 && p_count <= -3)
                            || self.wager * 2.0 > self.funds
                        {
                            self.hit(deck, false);
                        } else {
